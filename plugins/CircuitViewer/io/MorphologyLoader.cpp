...
const Property PROP_LAYOUT_NB_COLUMNS = {"layoutNbColumns",
                                         "Layout: Number of columns", 0};
const Property PROP_LAYOUT_VERTICAL_SPACING = {"layoutVerticalSpacing",
                                               "Layout: Vertical spacing", 0};
const Property PROP_LAYOUT_HORIZONTAL_SPACING = {"layoutHorizontalSpacing",
...
bool _isInMask(const brain::neuron::SectionType type, const size_t mask)
{
    ...
    default:
        return true; // What do we do here?
    }
}

/* Morphology tree processing */

uint32_ts computeTreeTraversalOrder(brain::neuron::Morphology& morphology,
    ...
    const LoaderPropertyMap& properties)
{
    ...
    setVariable(layout.nbColumns, PROP_LAYOUT_NB_COLUMNS.name, 0);
    setVariable(layout.verticalSpacing, PROP_LAYOUT_VERTICAL_SPACING.name, 0);
    setVariable(layout.horizontalSpacing, PROP_LAYOUT_HORIZONTAL_SPACING.name,
                0);
...
public:
        ...
        const brain::CompartmentReportMapping* reportMapping) const
    {
        ...
        if (sectionMask & static_cast<size_t>(MorphologySectionType::soma))
        {
            ...
            _addSomaGeometry(morphology.getSoma(), offset,
                             useSDFGeometries, materialFunc, model, sdfData);
        }
             ...
             computeTreeTraversalOrder(morphology, sectionMask))
        {
            ...
            float previousRadius = _getCorrectedRadius(
                section.hasParent() ? section.getParent()[-1][3] * 0.5
                                    : samples[0].w() * 0.5);
                    ...
                    if (section.getID() < counts.size())
                    {
                        ...
                        else
                        {
                                ...
                                brain::neuron::SectionType::axon)
                            {
                                ...
                                // This should never happen, but jus in case
    ...
    const size_t defaultMaterialId BRAYNS_UNUSED) const
{
    ...
    impl.processMorphology(morphology, index).dump(model);
    ...
    const
{
    ...
    pm.setProperty(PROP_LAYOUT_NB_COLUMNS);
    pm.setProperty(PROP_LAYOUT_VERTICAL_SPACING);
    pm.setProperty(PROP_LAYOUT_HORIZONTAL_SPACING);
