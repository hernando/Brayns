...
struct CircuitProperties
{
    ...
    CircuitProperties(const LoaderPropertyMap& properties)
    {
        ...
        const auto setEnumVariable =
            [&](auto& variable, const std::string& name,
                ...
                using T =
                    typename std::remove_reference<decltype(variable)>::type;
                variable =
        ...
        boundingBox =
            Boxd(arrayToVmmlVec(boundingBoxP0), arrayToVmmlVec(boundingBoxP1));
    }
...
{
public:
    Impl(Scene& scene, const LoaderPropertyMap& properties)
        : _scene(scene)
...
std::pair<std::string, PropertyMap> CircuitLoader::getLoaderProperties() const
{
    ...
    { // Add all morphology loader properties
        const auto mlpm = MorphologyLoader(_scene).getLoaderProperties().second;
