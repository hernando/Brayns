    ...
    const size_t defaultMaterialId) const
{
    const auto geometryQuality =
        properties.getEnumProperty<GeometryQuality>(PROP_GEOMETRY_QUALITY.name,
                                                    PROP_GEOMETRY_QUALITY.enums,
                                                    GeometryQuality::high);
    const auto colorScheme =
        properties.getEnumProperty<ColorScheme>(PROP_COLOR_SCHEME.name,
                                                PROP_COLOR_SCHEME.enums,
                                                ColorScheme::none);
...
ModelDescriptorPtr MeshLoader::importFromBlob(
    ...
    const size_t defaultMaterialId) const
{
    const auto geometryQuality = properties.getEnumProperty<GeometryQuality>(
        PROP_GEOMETRY_QUALITY.name, enumerateGeometryQualityEnums(),
        GeometryQuality::high);
    const auto colorScheme =
        properties.getEnumProperty<ColorScheme>(PROP_COLOR_SCHEME.name,
...
LoaderSupport MeshLoader::getLoaderSupport() const
{
    return {LOADER_NAME, getSupportedTypes()};
}
std::pair<std::string, PropertyMap> MeshLoader::getLoaderProperties() const
{
    PropertyMap pm;
