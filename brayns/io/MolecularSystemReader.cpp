...
bool MolecularSystemReader::_createScene(
    ...
    const LoaderPropertyMap& properties) const
{
    const auto colorScheme =
        properties.getEnumProperty<ColorScheme>(PROP_COLORSCHEME.name,
                                                PROP_COLORSCHEME.enums,
                                                ColorScheme::none);
...
LoaderSupport MolecularSystemReader::getLoaderSupport() const
{
    return {LOADER_NAME, {"molsys"}};
}
std::pair<std::string, PropertyMap> MolecularSystemReader::getLoaderProperties()
    const
{
    PropertyMap pm;
    ...
    { // Add all mesh loader properties
        const auto mlpm = MeshLoader(_scene).getLoaderProperties().second;
