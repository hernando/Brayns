...
const Property PROP_TYPE = {"type", "Type", int32_t(brayns::DataType::UINT8),
                            brayns::enumerateDataTypeEnums()};
}
...
ModelDescriptorPtr RawVolumeLoader::importFromBlob(
    ...
    const LoaderPropertyMap& properties BRAYNS_UNUSED,
...
ModelDescriptorPtr RawVolumeLoader::importFromFile(
    ...
    const LoaderPropertyMap& properties, const size_t index BRAYNS_UNUSED,
    const size_t defaultMaterialId BRAYNS_UNUSED) const
{
    callback.updateProgress("Parsing volume file ...", 0.f);

    const auto dimensions =
        arrayToVmmlVec(properties.getProperty(PROP_DIMENSIONS.name,
                                              std::array<int32_t, 3>{0, 0, 0}));
    const auto spacing =
        arrayToVmmlVec(properties.getProperty(PROP_SPACING.name,
                                              std::array<double, 3>{0, 0, 0}));
    const auto type =
        properties.getEnumProperty<DataType>(PROP_TYPE.name, PROP_TYPE.enums,
                                             DataType::UINT8);
...
LoaderSupport RawVolumeLoader::getLoaderSupport() const
{
    return {"raw-volume", {"raw"}};
}
std::pair<std::string, PropertyMap> RawVolumeLoader::getLoaderProperties() const
{
    PropertyMap pm;
    pm.setProperty(PROP_DIMENSIONS);
    pm.setProperty(PROP_SPACING);
    pm.setProperty(PROP_TYPE);
    return {"raw-volume", pm};
}
////////////////////////////////////////////////////////////////////////////
...
ModelDescriptorPtr MHDVolumeLoader::importFromFile(
    const std::string& filename, const LoaderProgress& callback,
    const LoaderPropertyMap&, const size_t index BRAYNS_UNUSED,
    const size_t defaultMaterialId BRAYNS_UNUSED) const
{
    callback.updateProgress("Parsing volume file ...", 0.f);
    ...
    LoaderPropertyMap properties;
    properties.properties["dimensions"].push_back(
        {PROP_DIMENSIONS.name, PROP_DIMENSIONS.label, dimensions});
    properties.properties["spacing"].push_back(
        {PROP_SPACING.name, PROP_SPACING.label, spacing});
    properties.properties["type"].push_back(
        {PROP_TYPE.name, PROP_TYPE.label, int32_t(type), PROP_TYPE.enums});
...
LoaderSupport MHDVolumeLoader::getLoaderSupport() const
{
    return {"mhd-volume", {"mhd"}};
}
std::pair<std::string, PropertyMap> MHDVolumeLoader::getLoaderProperties() const
{
    return {"mhd-volume", {}};
}
}
